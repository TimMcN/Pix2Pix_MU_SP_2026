...
class Pix2PixMUModel(BaseModel):
    ...
    def backward_D(self):
        ...
        self.real_A_e = self.real_A.expand(-1,-1,self.fake_B.shape[2], -1)
        fake_AB = torch.cat((self.real_A_e, self.fake_B), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        pred_fake = self.netD(fake_AB.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake, False)
        # Real
        real_AB = torch.cat((self.real_A_e, self.real_B), 1)
    ...
    def backward_G(self):
        ...
        self.real_A_e = self.real_A.expand(-1,-1,self.fake_B.shape[2], -1)
        fake_AB = torch.cat((self.real_A_e, self.fake_B), 1)
